...
/* ************************************************************
                      GpsService
Get and update motorista current location in server
*************************************************************** */
class GpsService : Service(), ServiceListener {
    ...
    override fun onCreate() {
        ...
        complexPreferences =
            ComplexPreferences.getComplexPreferences(this, "mypref", Context.MODE_PRIVATE)
        ...
        mFirebaseDatabaseRequest =
            mFirebaseInstance.getReference(applicationContext.getString(R.string.real_time_db))
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult) {
                ...
                if (sessionManager.tripId != null && sessionManager.tripId != "") {
                    ...
                    CommonMethods.DebuggableLogE(
                        TAG,
                        "Driver LOCATION_AND_WRITEPERMISSION_ARRAY data removed!"
                    )
...
*/
    ...
    private fun checkButtonServiceStatus(isRunning: Boolean) {
        try {
            if (isRunning) {
                Log.i(
                    TAG,
                    "checkButtonServiceStatus: WHAT=${
                        commonMethods.isMyServiceRunning(
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
                    if (!commonMethods.isMyServiceRunning(FloatingWidgetService::class.java, this))
                        startService(
                            Intent(
                                this,
                                FloatingWidgetService::class.java
            ...
            Log.i(TAG, "checkButtonServiceStatus: Error=${e.localizedMessage}")
        }
    }
        ...
        if (CommonKeys.driverInActive === 1) {
            ...
            if (currentLocation != null) {
                ...
                if (distanceInMeters > 15) {
                    if (sessionManager.userId != null && !sessionManager.userId.equals("") && sessionManager.driverStatus.equals(
                            "Online",
                            ignoreCase = true
                        ) && !sessionManager.isTrip
                    ) {
                        val ref = FirebaseDatabase.getInstance().reference.child(
                            applicationContext.getString(R.string.real_time_db)
    ...
    fun locationupdateCall() {
        if (!TextUtils.isEmpty(sessionManager.currentLatitude) && !TextUtils.isEmpty(sessionManager.currentLongitude)) {
            ...
            CommonMethods.DebuggableLogE(
                "location ",
                "Update Call currentLocation:" + +currentLocation!!.latitude + " , " + currentLocation!!.longitude
            )
            if (sessionManager.isTrip) {
                AddFirebaseDatabase().removeDriverFromGeofire(this@GpsService)
            } else {
   ...
   */
    fun updateLocationChange() {
        Log.i(TAG, "updateLocationChange:GPS Service Called updateLocationChange")
            ...
            if (tripStatus != null && tripStatus == beginTrip) {
                ...
                try {
                    distance = java.lang.Float.valueOf(
                        twoDForm.format(distance.toDouble()).replace(",".toRegex(), ".")
                    )
                } catch (e: Exception) {
                    e.printStackTrace()
                }
                ...
                CommonMethods.DebuggableLogE(
                    "locationupdate",
                    "Distance:" + distance + "Max Distance" + MAX_DISTANCE
                )
                if (distance * 1000 < MAX_DISTANCE) {
                    val newLatLng = LatLng(
                        java.lang.Double.valueOf(sessionManager.latitude!!),
                    ...
                    if (isInternetAvailable) {
                        ...
                        CommonMethods.DebuggableLogE(
                            "locationupdate",
                            "Offline Distance:" + (sessionManager.offlineDistance + distance)
                        )
                        ...
                        val message =
                            (sessionManager.offlineDistance.toString() + " " + tripStatus + " " + sessionManager.onlineDistance + " "
                ...
                duration = 30
                if (sessionManager.driverStatus != null && sessionManager.driverStatus == resources.getString(
                        R.string.online
                    )
                ) {
                    val locationHashMap = HashMap<String, String>()
    ...
    /**
     * Update motorista current location
     */
    fun updateLocation(locationHashMap: HashMap<String, String>) {
        ...
        if (isInternetAvailable) {
            ...
            val time = now.hour.toString() + ":" + now.minute + ":" + now.second
            val message =
                sessionManager.offlineDistance.toString() + " " + sessionManager.tripStatus + " " + sessionManager.onlineDistance + " " + sessionManager.latitude + "," + sessionManager.longitude + " " + time


            apiService.updateLocation(locationHashMap)
                .enqueue(RequestCallback(REQ_UPDATE_ONLINE, this))
        } else {
  ...
  */
    ...
    private fun startMyOwnForeground() {
        val NOTIFICATION_CHANNEL_ID = "com.rideincab.driver"
    ...
    override fun onDestroy() {
        ...
        if (str != null && str == str2) {
            ...
            val broadcastIntent =
                Intent(this, GPSBackgroundServiceRestarterBroadcastReceiver::class.java)
    ...
    private fun updateLocationFireBase(lat: Double, lng: Double) {
        ...
        for (i in poolIds.indices) {
            if (poolIds.get(i) != null && !poolIds.get(i).equals(""))
                mFirebaseDatabase!!.child(FirebaseDbKeys.LIVE_TRACKING_NODE).child(poolIds.get(i))
    ...
    private fun addRequestReceiveListner() {
        ...
        if (::mFirebaseDatabaseRequest.isInitialized) {
            ...
            queryRequest = mFirebaseDatabaseRequest.child(FirebaseDbKeys.TRIP_REQUEST)
                .child(sessionManager.userId!!)
            mSearchedRequestReferenceListener =
                queryRequest.addValueEventListener(object : ValueEventListener {
            ...
            if (!NotificationUtils.isAppIsInBackground(applicationContext)) {
                ...
                if (json.getJSONObject("custom").has("ride_request")) {
                    ...
                    if (DriverStatus == "Online"
                        ...
                        val title = json.getJSONObject("custom").getJSONObject("ride_request")
                            .getString("title")
    ...
    private fun addLatLngChangeListener() {
        ...
        /*if(mSearchedLocationReferenceListener!=null)
            mFirebaseDatabase.removeEventListener(mSearchedLocationReferenceListener);*/
        // User data change listener
        val query = mFirebaseDatabase!!.child(FirebaseDbKeys.LIVE_TRACKING_NODE)
            .child(sessionManager.tripId!!)
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    if (sessionManager.tripId != null) {
                        ...
                        if (driverLocation == null) {
                            CommonMethods.DebuggableLogE(
                                TAG,
                                "Driver LOCATION_AND_WRITEPERMISSION_ARRAY data is null!"
                            )
                            return
                        }
                        ...
                        CommonMethods.DebuggableLogE(
                            TAG,
                            "Driver LOCATION_AND_WRITEPERMISSION_ARRAY data is changed!" + driverLocation.lat + ", " + driverLocation.lng
                        )
                    } else {
 ...
 */
    ...
    companion object {
        ...
        val LOCATION_INTERVAL =
            1000 * 1 * 1 // 1000 * 60 * 1 for 1 minute 1000 * 10 * 1 for 10 seconds
        val LOCATION_INTERVAL_CHECK = 1000 * 1 * 1
        val LOCATION_UPDATE_INTERVAL =
            1000 * 10 * 1 // 1000 * 60 * 1 for 1 minute 1000 * 10 * 1 for 10 seconds
