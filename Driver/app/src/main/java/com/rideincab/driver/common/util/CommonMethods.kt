...
import com.rideincab.driver.home.firebaseChat.FirebaseChatNotificationService
    ...
    fun handleDataMessage(json: JSONObject, context: Context) {
        ...
        try {
            val mFirebaseDatabase = FirebaseDatabase.getInstance()
                .getReference(context.getString(R.string.real_time_db))
            mFirebaseDatabase!!.child(FirebaseDbKeys.Notification).child(sessionManager.userId!!)
                .removeValue()
...
//            if (!NotificationUtils.isAppIsInBackground(context)) {
            //Default True is set to open booking if app is in background or in forground
            if (true) {
                ...
                try {
                    ...
                    if (jsonObject.getJSONObject("custom").has("ride_request")) {
                        isPool = json.getJSONObject("custom").getJSONObject("ride_request")
                            .getBoolean("is_pool")
                        requestId = jsonObject.getJSONObject("custom").getJSONObject("ride_request")
                        ...
                        if (json.getJSONObject("custom").has("ride_request")) {
                            if (DriverStatus == "Online"
                                ...
                                val requstreceivepage =
                                    Intent(context, RequestReceiveActivity::class.java)
                ...
                if (json.getJSONObject("custom").has("cancel_trip")) {
                    ...
                    sessionManager.dialogMessage =
                        context.resources.getString(R.string.yourtripcanceledrider)
                    dialogs.putExtra(
                        "message",
                        context.resources.getString(R.string.yourtripcanceledrider)
                    )
                    ...
                    val riderProfile = json.getJSONObject("custom").getJSONObject("trip_payment")
                        .getString("rider_thumb_image")
                    ...
                    dialogs.putExtra(
                        "message",
                        context.resources.getString(R.string.paymentcompleted)
                    )
                    dialogs.putExtra("type", 1)
                    ...
                    val message = json.getJSONObject("custom").getJSONObject("custom_message")
                        .getString("message_data")
                    val title = json.getJSONObject("custom").getJSONObject("custom_message")
                        .getString("title")
                    ...
                    manualBookingTripBookedInfo(
                        ...
                        json.getJSONObject("custom")
                            .getJSONObject("manual_booking_trip_canceled_info"),
                        context
                    )
                } else if (json.getJSONObject("custom").has("manual_booking_trip_assigned")) {
                    manualBookingTripStarts(
                        json.getJSONObject("custom").getJSONObject("manual_booking_trip_assigned"),
                        context
                    )
                    ...
                    try {
                        ...
                        val tripId = json.getJSONObject("custom").getJSONObject("chat_notification")
                            .getString("trip_id")
...
//                        context.startActivity(notificationIntent)
                        ...
                        Log.i(TAG, "handleDataMessage: Error=${e.localizedMessage}")
                    }
                } else {
                ...
                if (json.getJSONObject("custom").has("ride_request")) {
                    ...
                    val isPool = json.getJSONObject("custom").getJSONObject("ride_request")
                        .getBoolean("is_pool")
                    ...
                    if (DriverStatus == "Online"
                        ...
                        try {
                            ...
                            if (!sessionManager.requestId.equals(requestId)) {
                                ...
                                var msg = json.getJSONObject("custom").getJSONObject("ride_request")
                                    .getString("title")
                                ...
                                val endtime =
                                    jsonObject.getJSONObject("custom").getString("end_time")

                                val duration =
                                    difference(getCurrentTimeIntoLong(), endtime.toLong())
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && NotificationUtils.isAppIsInBackground(
                                    ...
                                    RequestEndTime =
                                        json.getJSONObject("custom").getString("end_time")
                                    val timeStamp =
                    ...
                    CommonKeys.keyCaller =
                        json.getJSONObject("custom").getJSONObject("user_calling")
                            .getString("title")
                    initSinchService(context)
                } else if (json.getJSONObject("custom").has("chat_notification")) {
                    ...
                    val notificationUtils = NotificationUtils(context)
                    notificationUtils.playNotificationSound()
                    val message = json.getJSONObject("custom").getJSONObject("chat_notification")
                        .getString("message_data")
                    //val title = context.getString(R.string.app_name)
        ...
        fun getAppVersionNameFromGradle(context: Context): String {
            ...
            try {
                versionName = AppController.context!!.packageManager
                    .getPackageInfo(AppController.context!!.packageName, 0).versionName.toString()
...
//            mContext.stopService(Intent(mContext, FirebaseChatNotificationService::class.java))
        ...
        fun getCustomHeaderForAlert(mContext: Context, title: String): TextView {
            ...
            headerFontTextView.setTypeface(
                FontCache.getTypeface(
                    mContext.resources.getString(R.string.fonts_UBERMedium),
                    mContext
                ), Typeface.BOLD
            )
        ...
        fun playVibration() {
            try {
                val v =
                    AppController.context!!.getSystemService(Context.VIBRATOR_SERVICE) as Vibrator
                // Vibrate for 500 milliseconds
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    v.vibrate(VibrationEffect.createOneShot(500, VibrationEffect.DEFAULT_AMPLITUDE))
                } else {
        ...
        fun showInternetNotAvailableForStoredDataViewer(mContext: Context) {
            ...
            val toast: Toast = Toast.makeText(
                mContext,
                mContext.resources.getString(R.string.you_are_viewing_old_data),
                Toast.LENGTH_LONG
            )
        ...
        fun showNoInternetAlert(
            context: Context,
            iNoInternetCustomAlertCallBack: INoInternetCustomAlertCallback
        ) {
            val builder = AlertDialog.Builder(context)
    ...
    fun getClientSecret(jsonResponse: JsonResponse, activity: AppCompatActivity) {
        ...
        if (stripeInstance() != null) {
            stripeInstance()!!.confirmPayment(
                activity,
                createPaymentIntentParams(clientSecret, activity.applicationContext)
            )
        } else {
            hideProgressDialog()
            Toast.makeText(
                activity.applicationContext,
                activity.applicationContext.resources.getString(R.string.internal_server_error),
                Toast.LENGTH_SHORT
            ).show()
        }
    }
    ...
    fun manualBookingTripBookedInfo(
        manualBookedPopupType: Int,
        ...
        try {
            riderName =
                jsonObject.getString("rider_first_name") + " " + jsonObject.getString("rider_last_name")
            riderContactNumber =
                jsonObject.getString("rider_country_code") + " " + jsonObject.getString("rider_mobile_number")
        ...
        dialogs.putExtra(
            CommonKeys.KEY_MANUAL_BOOKED_RIDER_PICKU_DATE_AND_TIME,
            riderPickupDateAndTime
        )
    ...
    fun createFileAndUpdateDistance(context: Context) {
        try {
            val distanceFile = File(
                context.getExternalFilesDir(Environment.DIRECTORY_PICTURES),
                context.getResources().getString(R.string.app_name) + "_TripDistanceCalculation"
            )
    ...
    fun finalDistanceUpdateInFile(
        googleDistance: Float,
        distanceCalculatedEvery1Sec: Float,
    ...
    fun getTimeFromLong(time: Long): String {
        ...
        val sdf =
            SimpleDateFormat("dd MMM yyyy HH:mm:ss", Locale.ENGLISH) // the format of your date
    ...
    fun increaseVolume(context: Context) {
        try {
            ...
            Log.i("increaseVolume", "increaseVolume: Error=${e.localizedMessage}")
        }
