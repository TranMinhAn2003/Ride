...
class ImageCompressAsyncTask(
    ...
    override fun doInBackground(vararg params: Void): Void? {
        try {
            ...
            if (file.exists()) {
                ...
                runBlocking {
                    compressPath = compressFile(file)
                }

                // Determine document type and upload type
                docType = when (type) {
                    1 -> "license_back"
                    2 -> "license_front"
                    3 -> "insurance"
    ...
    private suspend fun compressFile(file: File): String = withContext(Dispatchers.IO) {
        val compressedFile = Compressor.compress(compressImgWeakRef!!.get()!!, file) {
