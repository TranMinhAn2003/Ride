            ...
            if (response != null) {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    if (BuildConfig.DEBUG)
                       Log.i("API_CALLS", "getSuccessResponse: strJson=$strJson")
                    ...
                    if (response.errorBody()!=null){
                        ...
                        if (BuildConfig.DEBUG)
                           Log.i("API_CALLS", "getSuccessResponse: errorBody=$strJson")

                        jsonResp.errorMsg = strJson
                        jsonResp.strResponse = strJson
                        jsonResp.statusMsg = this.getStatusMsg(strJson)!!

                        if (jsonResp.statusMsg.equals("Token Expired", ignoreCase = true) && strJson.contains("refresh_token")) {
                            jsonResp.statusMsg = ""
                            ...
                            try {
                                ...
                                Log.e(
                                    "RequestCallBack",
                                    "getSuccessResponse: 401,404 Error=${e.localizedMessage}",
                                )
                                val intent = Intent(context, SigninSignupHomeActivity::class.java)
                                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_NEW_TASK)
                                intent.putExtra("clearservice", true)
