   ...
   */
    override fun onBindViewHolder(viewHolder: CommentsRecycleAdapter.ViewHolder, i: Int) {
        ...
        val languageLocaleMap = mapOf(
            ...
            "ar" to Locale("ar", "DZ"),
            "pt" to Locale("pt", "PT"),
            "iw" to Locale("iw", "IL"),  // Hebrew (iw is legacy code; modern is "he")
            "fr" to Locale("fr", "FR"),
            "bg" to Locale("bg", "BG")
        )

        val selectedLocale = languageLocaleMap[sessionManager.languageCode] ?: Locale.getDefault()
        val targetFormat = SimpleDateFormat("dd MMMM yyyy", selectedLocale)
